classdef IO_SVD_BHTMM < handle
    ...
    properties
        ...
        alpha_pr;
        alpha_em;             
        pr_x_tab;   % the table P(Q | x)
        em_y_tab;   % the table P(y | Q, x)
    ...
    methods
        ...
        function obj = IO_SVD_BHTMM(C_,L_,Mx_, My_, min_lag_, max_lag_, alpha_zero_,alpha_lamb_,gamma_lag_,phi_exp_,alpha_pr_,alpha_em_)           
            obj.C = C_+1;
            obj.bottomCh = obj.C;
            obj.L = L_;
            obj.Mx = Mx_;
        ...
        function [likelihood_vect, time_vect, K_vect] = train(obj,X_list,Y_list,max_it, varargin)
            ...
            obj.initialise_before_inference(X_list,Y_list);
            ...
            for it=1:max_it
                ...
                tic;
                obj.one_step_inference(X_list,Y_list, current_temp, false);
                t = toc;
        ...
        function [likelihood_vect, Q_vect, Y_vect, time_vect, K_vect] = test(obj,X_list,max_it, varargin)
            ...
            for it=1:max_it
                ...
                tic;
                obj.one_step_inference(X_list,[], current_temp, true);
                t = toc;
            ...
            Y_vect = cell(N,1);
            for i=1:N
                Y_vect{i} = obj.sample_visible_y(X_list{i},obj.Q_traj{i}); 
            end
        end
                        
        function one_step_inference(obj,X_list,Y_list, current_temp, is_test)
            ...
            for j=1:N
                ...
                if is_test
                    ...
                    [obj.Q_traj{j}, obj.Z_traj{j}, obj.loglike_trees(j), scart] = obj.sample_latent_Q_and_Z(X_list{j},Y_list{j}, obj.Q_traj{j}, obj.Z_traj{j}, obj.loglike_trees(j), current_temp);
                end
            ...
            if(~is_test)
                fprintf(1,"N seq scartate: %d\n",n_scart);           
                obj.sample_values(X_list,Y_list, current_temp);
                fprintf(1,"K values: ")
                for ll=1:obj.L
                    fprintf(1,"%d ",obj.k_values(ll));
                end
                fprintf(1,"\n");
            end
        end
        
        function [Q_new, Z_new, loglike_new, scart] = sample_latent_Q_and_Z(obj, tree_obj, y_vect, Q_old, Z_old, loglike_old, current_temp)
            ...
            for u=n:-1:n-nf+1                
                ...
                if y_u ~= -1
                    prob = obj.pr_x_tab(x_u,:) .* reshape(obj.em_y_tab(x_u,:,y_u),[1 C_]);
                else
                    prob = obj.pr_x_tab(x_u,:);
                end
            ...
            for u=n-nf:-1:1
                ...
                if y_u ~= -1
                    prob = obj.pr_x_tab(x_u,:) .* lamb_z .* reshape(obj.em_y_tab(x_u,:,y_u),[1 C_]);
                else
                    prob = obj.pr_x_tab(x_u,:) .* lamb_z;
                end
                Q_new(u) = catrnd(prob, 1);
        ...
        function sample_values(obj, X_list, Y_list, current_temp)
            L_ = obj.L;
            C_ = obj.C;
            Mx_ = obj.Mx;
            My_ = obj.My;
            ...
            % sample the new_k_values
            cont = 1;
            prob_to_sample_lag = ones(1,L_);
            ...
            while(cont)
                ...
                if(old_k_val==1)
                    ...
                    if(n_important_lags == max_lag_)
                        % we cannot do this move, so exclude l-th position
                        prob_to_sample_lag(l) = 0;
                        continue;
                    end
                elseif(old_k_val==C_)
                    new_k_val = C_-1;
                    ...
                    if(new_k_val == 1)
                        % the lag l-th becomes non-important
                        if(n_important_lags == min_lag_)
                            %we cannot do this move, so increment the value
                            new_k_val = old_k_val+1;
                        end
                    end
                end
                ...
                if(new_k_val > old_k_val)
                    ...
                    while(~any(el_new_cluster) || all(el_in_cluster==el_new_cluster))
                        el_new_cluster = logical((catrnd([1 1],C_) - 1)' .* el_in_cluster);
                    end
                ...
                if(~all(sum(new_pi_tab(l,:,1:new_k_val),2)))
                    aaa = 3;
                end
                
                n_important_lags = sum(new_k_values > 1);

                if(n_important_lags >= min_lag_ && n_important_lags<=max_lag_ )
                    % therea are important lag
                    cont = 0;
                end
            ...
            em_y_stats = zeros([Mx_ C_ My_]);
            pr_x_stats = zeros([Mx_ C_]);
            ...
            for i=1:N    
                ...
                for u=n-nf:-1:1
                    ...
                    em_y_stats(x_u,q_tmp(u),y_u) = em_y_stats(x_u,q_tmp(u),y_u)+1;
                end
                ...
                for u=n:-1:n-nf+1     
                    ...
                    pr_x_stats(x_u, q_tmp(u)) = pr_x_stats(x_u, q_tmp(u)) + 1;
                end
            ...
            for xx=1:Mx_
                for cc=1:C_-1
                    obj.em_y_tab(xx,cc,:) = drchrnd(reshape(em_y_stats(xx,cc,:),[1 My_]) + obj.alpha_em, 1);
                end
            end
            
            % sample priori
            for xx=1:Mx_
                obj.pr_x_tab(xx,1:end-1) = drchrnd(obj.alpha_pr + pr_x_stats(xx,1:end-1),1);
            end
        end
        
        function y_list = sample_visible_y(obj, tree_obj, Q_list)
            n = tree_obj.n;
            y_list= zeros(1,n);
            My_ = obj.My;
            for u=1:n
                q_u = Q_list(u);
                x_u = tree_obj.v(u);
                prob = reshape(obj.em_y_tab(x_u,q_u,:),[1 My_]);
                y_u = catrnd(prob,1);
                y_list(u) = y_u;
            end
        end
    ...
    methods(Access=private)
        ...
        function init_potentials_from_priori(obj)
            ...
            obj.pr_x_tab = zeros(Mx_, C_);
            obj.pr_x_tab(:,1:end-1) = drchrnd(obj.alpha_pr * ones(1,C_-1),Mx_);
                                                
            obj.em_y_tab = zeros(Mx_,C_,My_);
            aux = drchrnd(obj.alpha_em * ones(1,My_),Mx_ * (obj.C-1));
            obj.em_y_tab(:,1:end-1,:) = reshape(aux,[Mx_ C_-1 My_]);
        end
        
        function initialise_before_inference(obj, X_list, Y_list)
            N = length(X_list);
            L_ = obj.L;
            C_ = obj.C;
            ...
            for j=1:N
                ...
                for u=n-nf:-1:1
                    ...
                    if ~isempty(Y_list)
                        y_u = y_vect(u);
                        prob = obj.pr_x_tab(x_u,:) .* reshape(obj.lamb_tab(idx_lambda{:}),[1 C_]) .* reshape(obj.em_y_tab(x_u,:,y_u),[1 C_]);
                    else
                        prob = obj.pr_x_tab(x_u,:) .* reshape(obj.lamb_tab(idx_lambda{:}),[1 C_]);
                    end
